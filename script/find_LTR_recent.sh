...
echo "run LTRharvest to collect relatively recent LTRs that are 99% or more in similarity"
gt ltrharvest -index $SEQINDEX -out seqfile.out${SIM} -outinner seqfile.outinner${SIM} -gff3 seqfile.gff${SIM} -minlenltr 100 -maxlenltr 6000 -mindistltr 1500 -maxdistltr 25000 -mintsd 5 -maxtsd 5 -motif tgca -similar ${SIM} -vic 10  > seqfile.result${SIM}
